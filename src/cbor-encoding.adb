...
package body CBOR.Encoding is
   ...
   is
      H : constant SSE.Storage_Element :=
        Make_Head (MT, 0);
   begin
      if Val <= 23 then
         declare
            R : constant SSE.Storage_Array (1 .. 1) :=
              [1 => H + SSE.Storage_Element (Val)];
         begin
         ...
         declare
            R : constant SSE.Storage_Array (1 .. 2) :=
              [1 => H + 24,
               2 => To_SE (U8 (Val))];
         begin
         ...
         declare
            V : constant Unsigned_16 :=
              Unsigned_16 (Val);
            R : constant SSE.Storage_Array (1 .. 3) :=
              [1 => H + 25,
         ...
         declare
            V : constant Unsigned_32 :=
              Unsigned_32 (Val);
            R : constant SSE.Storage_Array (1 .. 5) :=
              [1 => H + 26,
         ...
         declare
            ...
            R : constant SSE.Storage_Array (1 .. 9) :=
              [1 => H + 27,
   ...
   procedure Append_All
     (Target : in out SSE.Storage_Array;
      Source : SSE.Storage_Array;
      From   : SSE.Storage_Offset)
   is
   begin
      for I in SSE.Storage_Offset range
        1 .. Source'Length
      loop
   ...
   function Encode_Byte_String
     (Data : SSE.Storage_Array)
      return SSE.Storage_Array
   is
      Header : constant SSE.Storage_Array :=
        Encode_Head (CBOR.MT_Byte_String, U64 (Data'Length));
      HL : constant SSE.Storage_Offset :=
        Header'Length;
      DL : constant SSE.Storage_Offset :=
        Data'Length;
      R  : SSE.Storage_Array (1 .. HL + DL) :=
        [others => 0];
   begin
   ...
   function Encode_Text_String
     (Text : String)
      return SSE.Storage_Array
   is
      Len : constant SSE.Storage_Offset :=
        Text'Length;
      Header : constant SSE.Storage_Array :=
        Encode_Head (CBOR.MT_Text_String, U64 (Len));
      HL : constant SSE.Storage_Offset :=
        Header'Length;
      R  : SSE.Storage_Array (1 .. HL + Len) :=
        [others => 0];
      Idx : SSE.Storage_Offset := HL + 1;
   begin
      Append_All (R, Header, 1);
      for C of Text loop
         pragma Loop_Invariant
         ...
         R (Idx) :=
           SSE.Storage_Element (Character'Pos (C));
         Idx := Idx + 1;
      end loop;
      return R;
   ...
   function Encode_Float_Half
     (Bytes : SSE.Storage_Array)
      return SSE.Storage_Array
   is
      H : constant SSE.Storage_Array (1 .. 1) :=
        [1 => Make_Head (CBOR.MT_Simple_Value, 0) + 25];
      R  : SSE.Storage_Array (1 .. 3) :=
        [others => 0];
   begin
      R (1) := H (1);
